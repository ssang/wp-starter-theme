...
export default function PlaceholderImage({
  ...
  return (
    <div className={`relative max-w-fit group/image ${ className } ${ ! image.id && 'p-4 bg-black/30' }`}>
      <MediaUpload
        onSelect={ onSelect }
        allowedTypes={ ['image'] }
        value={ image.id }
        render={({ open }) => (
          <>
            <Button
              iconSize={ 28 }
              className={[
                `absolute inset-0 opacity-0 w-full !h-full !m-0 grid place-items-center bg-blue-light/50 border-solid border-2 border-pink text-blue transition-opacity`,
            ...
            { image.url && (
              <Button
                iconSize={ 14 }
                className={[
                  `absolute top-2 right-2 z-50 p-1 !m-0 aspect-square rounded-full grid place-items-center bg-pink border-solid border-2 border-pink opacity-0 group-hover/image:opacity-100 transition text-yellow-light hover:text-blue `,
                ].join(' ')}
                icon="trash"
                label="Remove Image"
                onClick={ onDelete }
      ...
      { image.url
        ...
        : (
          <svg class={`${ placeholderClass } text-[#DADADA] group-hover/image:text-blue transition-colors`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 31 27" fill="none">
