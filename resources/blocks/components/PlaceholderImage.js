...
export default function PlaceholderImage({
  ...
  return (
    <div className={ _class({
      'relative group/image': true,
      'max-w-fit': image.id !== null,
      'p-4 flex justify-center items-center bg-black/30': ! image.id
    }) + ` ${className}` }>
      <MediaUpload
        onSelect={ onSelect }
        allowedTypes={ ['image'] }
        ...
        render={({ open }) => (
          <>
            <Button
              iconSize={ 28 }
              className={ _class({
                'absolute inset-0 opacity-0 w-full !h-full !m-0 grid place-items-center bg-red/50 border-solid border-2 border-red-dark text-white transition-opacity': true,
                'hover:opacity-100': image.id !== null,
              }) }
              icon={ image.id !== null ? 'edit' : 'format-image' }
              label={ image.id !== null ? 'Replace Image' : 'Add Image' }
              onClick={ open }
            />
            { (image.url && onDelete) && (
              <Button
                iconSize={ 14 }
                className={[
                  `absolute top-2 right-2 z-50 p-1 !m-0 aspect-square rounded-full grid place-items-center bg-red hover:bg-red-dark border-solid border-2 border-red opacity-0 group-hover/image:opacity-100 transition text-white hover:text-red `,
                ].join(' ')}
                icon="trash"
                label="Remove Image"
                onClick={ onDelete }
      ...
      />
      { image.url
        ? <img className={ imageClass } src={ image.url } style={ imageStyle } />
        : (
          <svg class={`${ placeholderClass } max-w-20 text-[#DADADA] group-hover/image:text-black transition-colors`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 31 27" fill="none">
